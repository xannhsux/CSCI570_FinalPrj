...
def nw_score(x, y,gap_penalty,mismatch_penalty):
        """calculate points gained at last line"""
        m = len(x)
        n = len(y)
        ...
        for i in range(1, m+1):
            ...
            for j in range(1, n+1):
                match = prev[j-1] + mismatch_penalty[x[i-1]][y[j-1]]
                delete = prev[j] + gap_penalty
                insert = curr[j-1] + gap_penalty
                curr[j] = min(match, delete, insert)
            prev = curr.copy()
            #copy the current into the previous column to rowl the 2-column matrix
        return prev
        
def hirschberg_helper(x, y, gap_penalty, mismatch_penalty):
    m = len(x)
    n = len(y)

    if m == 0:
        return ('_' * n, y, gap_penalty * n)
    elif n == 0:
        return (x, '_' * m, gap_penalty * m)
    elif m == 1 or n == 1:
        cost, X_alignment, Y_alignment = dp_basic(x, y, gap_penalty, mismatch_penalty)
        return (''.join(X_alignment), ''.join(Y_alignment), cost)

    mid = m // 2 #divide into int, not float
    x_left = x[:mid]
    x_right = x[mid:]

    score_left = nw_score(x_left, y, gap_penalty, mismatch_penalty)

    score_right = nw_score(x_right[::-1], y[::-1], gap_penalty, mismatch_penalty)
    score_right = score_right[::-1]

    total = [score_left[j] + score_right[j] for j in range(n+1)]
    min_score = min(total)
    split = total.index(min_score)

    left = hirschberg_helper(x_left, y[:split], gap_penalty, mismatch_penalty)
    right = hirschberg_helper(x_right, y[split:], gap_penalty, mismatch_penalty)

    total_cost = left[2] + right[2]

    return (left[0] + right[0], left[1] + right[1], total_cost)
...
def efficient_algo(input_file, output_file):
    ...
    aligned_x, aligned_y, dp_cost = hirschberg_helper(X, Y, delta, alpha)
    current, peak = tracemalloc.get_traced_memory()
    tracemalloc.stop()
    end_time = time.perf_counter()
    time_taken = (end_time - start_time) * 1000
    memory_consumed = peak/1024
...
if __name__ == "__main__":
    ...
    efficient_algo(input_file, output_file)
